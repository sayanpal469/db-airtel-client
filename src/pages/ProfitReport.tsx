...
export const ProfitReport = () => {
  ...
  const fetchAnalytics = async () => {
    ...
    setError('');
    try {
      const response = await payoutApi.getAnalytics(selectedYear);
      if (response.success) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition-all shadow-lg shadow-red-200"
          >
            <Download size={18} />
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    stroke="none"
                  >
                    {data?.packageProfit?.map((entry: any, index: number) => (
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data?.retailerRevenue} layout="vertical">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data?.engineerPerformance}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12}} />
                  <YAxis axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12}} tickFormatter={(val) => Math.floor(val).toString()} />
                  <Tooltip 
