...
import { getRetailerPayouts } from '../utils/finance';
...
export const RetailerPayouts = () => {
  ...
  // Initialize payouts
  useMemo(() => {
    setPayouts(getRetailerPayouts(DUMMY_CONNECTIONS, selectedMonth, selectedYear));
              ...
              <tbody className="divide-y divide-slate-50">
                {selectedPayout?.connections.map((conn) => (
                  <tr key={conn.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs font-bold text-slate-800 text-right">₹{conn.retailerCommission}</td>
                  </tr>
                ))}
              </tbody>
            </table>
